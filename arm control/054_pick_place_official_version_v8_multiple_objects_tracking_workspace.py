...
tempo_iniziale = rospy.get_time()  # 

durata_desiderata = 5  #  in secondi

rate = rospy.Rate(1)  # Frequenza di aggiornamento di 1 Hz 
max_count=0
while not rospy.is_shutdown():
    tempo_corrente = rospy.get_time()  
    tempo_trascorso = tempo_corrente - tempo_iniziale
    
    if tempo_trascorso >= durata_desiderata:
        break  # Esci dal ciclo 
    data=rospy.wait_for_message("/pose", String, timeout=10)
    #print(len(data.data.split("#")))
...
rospy.loginfo(max_count)



count=0
while True:
    rospy.loginfo("picking attempt: "+str(count))

    #AVVICINAMENTO
    move_group.go(taoi2, wait=True)
    #ATTENDO COMPUTAZIONE DELLE POSE
    rospy.sleep(3)
    
    #RICERCA ID DEGLI OGGETTI
    try:
        data=rospy.wait_for_message("/pose", String, timeout=10)
        object_list=[]
        ...
        for p in poses_list_string:
            pose=p.split(",")
            pose_item={'track_id': int(pose[0])}
            object_list.append(pose_item)
        
        #SCELGO OGGETTO TRAMITE INDEX COUNT
        #print(object_list[count]['track_id'])
        tid=object_list[0]['track_id']
        obj="/obj_detected_"+str(tid)
        print(obj)
    except:
        print("NO ITEM")
        break
    ...
    try:
        now = listener.getLatestCommonTime(obj, '/base_link')
        (trans,rot) = listener.lookupTransform('/base_link',obj, now)
    ...
    if(trans[1]<-0.4 or trans[1]>0.4): #outside workspace condition
        print("out of y-workspace")
        while(count<max_count): #search for another object
            try:
                count=count+1
                tid=object_list[count]['track_id']
                obj="/obj_detected_"+str(tid)
            except:
                print("provavo a cambiare oggetto ma non esiste un altro oggetto da prendere")
                break
            try:
                now = listener.getLatestCommonTime(obj, '/base_link')
                (trans,rot) = listener.lookupTransform('/base_link',obj, now)
                ...
                print("TRANSFORMATION NOT AVAILABLE: maybe there isn't any object")
                break
            if(trans[1]>-0.4 or trans[1]<0.4): #condition inside of workspace
                tid=object_list[count]['track_id']
                obj="/obj_detected_"+str(tid)
                print("L'OGGETTO CHE POSSO PRENDERE E': ",obj)
                break

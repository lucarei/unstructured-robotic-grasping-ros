...
data_angle = rospy.wait_for_message('/angle_talker', Float32, timeout=5)
#print(data_angle.data)
angle_rad=data_angle.data
angle_deg=angle_rad*180/pi

if angle_deg>69:
    print("go in central position")
    var=0
elif angle_deg<=21:
    print("go in full lateral position")
    var=pi/2
else:
    print("test two angles")
    for counter in range(2):
        joint_goal = move_group.get_current_joint_values()
        #joint_goal[5] = angle_rad-counter*pi/2
        a=pi/2-angle_rad
        if counter==0:
            joint_goal[5]=a
        else:
        ...
        print("Sending joint goal to find orientation...")
        move_group.go(joint_goal, wait=True)
        move_group.stop()
        rospy.sleep(2) 
        try:
            data = rospy.wait_for_message('/area_talker', Int32, timeout=5)
            print("area received: ", str(data.data))
            recordings.append(data.data)
        except rospy.ROSException:
            print("raise exception: YOLO problems")
            a=100000
            recordings.append(a)
        print("Goal successfull!")
    print(recordings)
    imin=recordings.index(min(recordings)) 
    if imin==0:
        var=a
    else:
        var=-a
    # print(angle_rad-imin*pi/2)
    # var=angle_rad-imin*pi/2
...
print("----------------------------->publishing angle:", str(var*180/pi))
pub_angle.publish(var)

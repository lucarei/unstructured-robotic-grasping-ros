...
class ObjDetector:
    ...
    def callback_pose(self, data):
        ...
        print(poses_list_string)
        for p in poses_list_string:
            pose=p.split(",")
            pose_item={'track_id': float(pose[0]),'x_center': float(pose[1]), 'y_center': float(pose[2]), 'angle': float(pose[3])}
        ...
        for i in range(len(self.object_list)):
            ...
            print(i)
            track_id=int(self.object_list[i]['track_id'])
            ...
            #print([X,Y,Z])
            frame="obj_detected_"+str(track_id)
